  ...
  for (country of countries) {
    ...
    countryItem = '<div id="'+country.code+'"class="flex-row dd-country">'+
        '<div class="flag">'+
          ...
          '<span>'+title+'</span>' +
          '</div>';
      if (country.state) { $(countryItem).insertAfter(".dd-country:last-child()"); }
	...
	function getChosenCountry(chosenCountry) {
		const name = chosenCountry.children("span").text();
		const country = {
					chosenCountryName : name,
					//saves name before underscore reinsert
					chosenCountryCopy : name,
		...
		// if EUU gets clicked default to florida
		if (country.chosenCountryName.toUpperCase() === "ESTADOS UNIDOS") {
			country.chosenCountryName = "florida";
			country.chosenCountryCopy = country.chosenCountryName;
			country.chosenCountryFlag = $("#FL")
			 														.children("div")
								      						.children("img")
								      						.attr("src")
		}
    ...
    $(".dd-country").click(function(e) {
      ...
      $(".countries-dropdown").addClass("pullup-fx");
	  	chosenCountry = $(this);
			const { chosenCountryName,
				      chosenCountryCopy,
						  chosenCountryFlag } = getChosenCountry(chosenCountry);
      ...
      snack.parent().hide();
      snack.parent().fadeIn();
      setTimeout(function(){snack.parent().fadeOut()}, 3000);
    });
...
const ENGLISH_URL = "https://www.webinads.com/cgi-bin/getstate.cgi";
...
function adjustQueriesToCountry( country ) {
  ...
  for (menuCat of allMenuCategories) {
	...
	//change country parameter on english page
	$(".english").attr("href", ENGLISH_URL + "?n=" + country.code);
